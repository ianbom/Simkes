...
const FeatureSection = () => {
    return (
        <section className="relative overflow-hidden bg-gradient-to-br from-white via-sky-50 to-sky-100 py-20">
            {/* Background effect */}
            <div className="absolute -bottom-40 right-1/4 h-96 w-96 rounded-full bg-sky-200/30 blur-3xl"></div>
            ...
            <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
                    {features.map((feature, index) => (
                        <div
                            key={index}
                            className="flex flex-col items-center rounded-2xl bg-white p-6 text-center shadow-md transition hover:shadow-lg"
                        >
                            <img
                                src={feature.image}
                                alt={feature.title}
