...
export default function GrafikBalita({ growth, child, activeTab }: any) {
    ...
    const calculateZScore = (value: number, percentileData: any[], month: number) => {
    ...
    const CustomTooltip = ({ active, payload }: any) => {
        if (active && payload && payload.length) {
            ...
            const measurement = currentConfig.measurements.find((m: any) => m.month === data.month);
        ...
        <div className="p-8 bg-white shadow-sm rounded-2xl">
            ...
            <ResponsiveContainer width="100%" height={450}>
                ...
                >
                    ...
                    {currentConfig.measurements.map((point: any, idx: number) => (
                        <ReferenceDot
                            key={idx}
                            x={point.month}
                            y={point.value}
                                          ...
                                          <div className="border rounded-lg bg-white shadow-sm p-4 space-y-4">
                                              ...
                                              <table className="min-w-full text-sm mb-3">
                                                <thead>
                                                  <tr className="border-b bg-gray-100">
                                                    <th className="px-3 py-2 text-left font-medium text-gray-600">Cara Ukur Tinggi</th>
                                                    <th className="px-3 py-2 text-left font-medium text-gray-600">Suhu (°C)</th>
                                                    <th className="px-3 py-2 text-left font-medium text-gray-600">Napas (/menit)</th>
                                                    <th className="px-3 py-2 text-left font-medium text-gray-600">Jantung (/menit)</th>
                                                    <th className="px-3 py-2 text-left font-medium text-gray-600">Oksigen (%)</th>
                                                  </tr>
                                                </thead>
                                                <tbody>
                                                  <tr className="border-b last:border-0">
                                                    <td className="px-3 py-2 text-gray-700">{item.cara_ukur_tinggi || '-'}</td>
                                                    <td className="px-3 py-2 text-gray-700">{item.suhu_tubuh_celsius || '-'}</td>
                                                    <td className="px-3 py-2 text-gray-700">{item.frekuensi_napas_per_menit || '-'}</td>
                                                    <td className="px-3 py-2 text-gray-700">{item.frekuensi_jantung_per_menit || '-'}</td>
                                                    <td className="px-3 py-2 text-gray-700">{item.saturasi_oksigen_persen || '-'}</td>
                                                  </tr>
                                                </tbody>
                                              </table>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                              <div className="p-3 bg-blue-50 rounded-lg border border-blue-100">
                                                <h4 className="font-medium text-blue-700 mb-1">Perkembangan Motorik</h4>
                                                <p className="text-sm text-gray-700 whitespace-pre-line">
                                                  {item.perkembangan_motorik || 'Belum diisi'}
                                                </p>
                                              </div>

                                              <div className="p-3 bg-teal-50 rounded-lg border border-teal-100">
                                                <h4 className="font-medium text-teal-700 mb-1">Perkembangan Kognitif</h4>
                                                <p className="text-sm text-gray-700 whitespace-pre-line">
                                                  {item.perkembangan_kognitif || 'Belum diisi'}
                                                </p>
                                              </div>

                                              <div className="p-3 bg-purple-50 rounded-lg border border-purple-100">
                                                <h4 className="font-medium text-purple-700 mb-1">Perkembangan Emosional</h4>
                                                <p className="text-sm text-gray-700 whitespace-pre-line">
                                                  {item.perkembangan_emosional || 'Belum diisi'}
                                                </p>
                                              </div>
                                            </div>

                                            {/* Catatan Pemeriksaan */}
                                            <div className="p-3 mt-2 bg-gray-50 border border-gray-200 rounded-lg">
                                              <h4 className="font-medium text-gray-800 mb-1">📝 Catatan Pemeriksaan</h4>
                                              <p className="text-sm text-gray-700 whitespace-pre-line">
                                                {item.catatan_pemeriksaan || 'Tidak ada catatan.'}
                                              </p>
                                            </div>

                                            {/* Riwayat Sakit (jika ada) */}
                                            <div className="mt-4">
                                              <h4 className="font-semibold text-gray-800 mb-2">🤒 Riwayat Sakit</h4>
                                              {riwayatSakit.length > 0 ? (
                                                  ...
                                                  <table className="min-w-full text-sm">
                                                    <thead>
                                                      <tr className="border-b bg-gray-100">
                                                        ...
                                                        <th className="px-3 py-2 text-left font-medium text-gray-600">Diagnosis</th>
                                                        <th className="px-3 py-2 text-left font-medium text-gray-600">Gejala</th>
                                                        <th className="px-3 py-2 text-left font-medium text-gray-600">Tindakan</th>
                                                        <th className="px-3 py-2 text-left font-medium text-gray-600">Catatan</th>
                                                      </tr>
                                                    </thead>
                                                    <tbody>
                                                      {riwayatSakit.map((rs: any, i: number) => (
                                                        <tr key={i} className="border-b last:border-0 hover:bg-gray-50">
                                                          <td className="px-3 py-2 text-gray-800">
                                                            {new Date(rs.tanggal_sakit).toLocaleDateString("id-ID")}
                                                          </td>
                                                          <td className="px-3 py-2 text-gray-700">{rs.diagnosis}</td>
                                                          <td className="px-3 py-2 text-gray-700">{rs.gejala}</td>
                                                          <td className="px-3 py-2 text-gray-700">{rs.tindakan_pengobatan}</td>
                                                          <td className="px-3 py-2 text-gray-700">{rs.catatan}</td>
                                                        </tr>
                                                      ))}
                                                    </tbody>
                                                  </table>
                                                ...
                                                <p className="text-sm text-gray-500 italic">
                                                  Tidak ada riwayat sakit pada kunjungan ini.
                                                </p>
                                              )}
                                            </div>
                                          </div>
